...
<head>
    ...
    <style>
        ...
        #student-data-editor.visible {
            display: block;
        }
        #student-data-editor h2 {
            margin-top: 0;
            margin-bottom: 15px;
            color: #007bff;
        }
        ...
        .descriptor-table tr:nth-child(even), .analysis-table tr:nth-child(even) {
        ...
        .participation-bar-wrapper .progress-bar-container {
            flex-grow: 1;
        }
        .participation-bar-wrapper .label, .participation-bar-wrapper .percentage {
            font-weight: bold;
            white-space: nowrap;
        }
        #history-view {
        ...
        #chart-container {
            border: 1px dashed #ccc; padding: 20px; text-align: center; background: white; min-height: 300px;
        }
        #saved-evaluations-list div:hover {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function saveCurrentEvaluationState() {
                ...
                if (!evaluationTitle) {
                    console.log("Guardado omitido: el título de la evaluación es necesario.");
                    return; 
                }
                ...
                const existingEvalIndex = allSavedEvaluations.findIndex(
                    eval => eval.title === evaluationData.title && eval.course === evaluationData.course
                );
                ...
                console.log('Evaluación guardada automáticamente!', evaluationData);
            ...
            function loadStudentsData(data) {
                ...
                for (const courseKey in data) {
                    if (Array.isArray(data[courseKey])) {
                        const sortedStudents = data[courseKey].sort((a, b) => {
                        ...
                        processedData[courseKey] = sortedStudents;
                    }
                }
            ...
            function renderDescriptorTable() {
                ...
                for (const indicator in ALL_INDICATORS_DATA) {
                    ...
                    row.innerHTML = `<td class="max-score-selection"><div class="score-buttons-selection"><button class="score-button" data-score="4">4</button><button class="score-button" data-score="2">2</button></div></td><td><strong>${indicator}</strong></td><td>${data[4]||""}</td><td>${data[3]||""}</td><td>${data[2]||""}</td><td>${data[1]||""}</td><td>${data[0]||""}</td>`;
            ...
            generateButton.addEventListener("click", () => {
                ...
                selectedIndicators.forEach(row => {
                    headerHTML += `<th>${row.dataset.indicator}</th>`;
                });
            ...
            function calculateStudentScores(row) {
                ...
                if (row.classList.contains("student-absent")) {
                    totalCell.textContent = "A";
                ...
                if (!isScored) {
                    totalCell.textContent = "S/I";
                    gradeCell.textContent = "S/I";
                    return;
                }
            ...
            function generateIndicatorAnalysis() {
                ...
                if (evaluatedCount === 0) {
                    analysisHTML += "<p>No hay estudiantes evaluados para analizar.</p>";
            ...
            // --- Lógica de la Vista de Historial ---
            ...
            function displaySavedEvaluations() {
                ...
                if (allEvaluations.length > 0) {
                    ...
                    allEvaluations.forEach(evaluation => {
                        ...
                        evalElement.style.background = '#e7f3fe';
                        evalElement.style.padding = '10px';
                        evalElement.style.borderRadius = '5px';
                        evalElement.style.marginBottom = '10px';
                        evalElement.style.cursor = 'pointer';
            ...
            function loadEvaluationState(evaluationId) {
                ...
                if (!evaluationToLoad) {
                    alert('Error: No se pudo encontrar la evaluación guardada.');
                    return;
                }
                ...
                document.querySelectorAll('.descriptor-table tr[data-indicator]').forEach(row => {
                    if(indicatorsInEval.includes(row.dataset.indicator)){
                        // Asumimos un puntaje máximo de 4 por ahora para la selección
                         row.querySelector(`.score-button[data-score='4']`)?.classList.add('selected');
                    }
                });
                                ...
                                for (const indicatorName in studentData.scores) {
                                    ...
                                    if (score !== null) {
                                        ...
                                        if (cell) {
                                            ...
                                            if (scoreButton) {
                                                // Simular click no funciona bien con addEventListener, se asigna clase directamente
                                                scoreButton.classList.add('selected');
                                            }
                                        }
                                    }
                                }
                                calculateStudentScores(row); // Recalcular puntajes después de asignar
                            }
                        }
                    });
                }, 100);
            ...
            function renderHistoryChart() {
                ...
                const chartData = filteredEvals.map(evaluation => {
                    ...
                    let maxScore = 4; // Asumir 4, se debe mejorar en el futuro
            ...
            function createLineChart(data) {
                ...
                if (points.length > 1) {
                    ...
                    let pathData = "M" + points.map(p => `${p.x},${p.y}`).join(" L");
            ...
            // Cargar listas de estudiantes y evaluaciones al iniciar
            displaySavedEvaluations();
        });
    </script>
</body>
